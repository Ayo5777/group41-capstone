...
DATA_MODULE_DIR = PROJECT_ROOT / "DataPart" / "data"

if str(DATA_MODULE_DIR) not in sys.path:
    sys.path.insert(0, str(DATA_MODULE_DIR))

from DataLoader import make_client_loaders
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--data_dir", type=str, default=str(DATA_MODULE_DIR))
    return p.parse_args()
...
def set_parameters(model: nn.Module, parameters: List[np.ndarray]) -> None:
    ...
    for key, parameter in zip(keys, parameters):
        tensor = torch.as_tensor(parameter, dtype=state_dict[key].dtype)
...
def main() -> int:
    ...
    trainloader, testloader = make_client_loaders(
